...
| **Windows 10 22H2, the whole cycle** | Windows 10 Enterprise Evaluation, build 19045. Backed up live in 209 seconds, 4.75 GB stored, verified, and a copy with a block missing refused. 11 of 11 hashes matched out of the backup. Restored from the wizard onto a blank disk: 14.5 GiB, four partitions. **It booted unaided**, and matched 12 of 12 hashes with the EFI partition intact and Windows RE registered |
...
| **The redesigned interface, the whole cycle** | Windows 10 22H2, build 19045. A backup taken from the rebuilt backup window: 17.2 GiB read, 7.8 GiB stored in 4m 15s, verified over 4,632 pieces. Recovery media built from the same build and booted into Windows PE, where the rebuilt wizard was **driven from the keyboard alone** - Enter to search, Down to choose a backup and a disk, the erase phrase typed, Enter to restore. 17.2 GiB written, four partitions, 1007.5 KiB left unallocated, and **the restored Windows started unaided**. Nothing was clicked inside the wizard |
| **The aligned interface, the whole cycle** | Windows 10 22H2, build 19045, on `0.1.0-alpha.2`. Backed up from the rebuilt window: 17.4 GiB read, 8.1 GiB stored in 5m 32s, verified over 5,567 pieces. Recovery media built from the same build, booted into Windows PE, and the wizard driven from the keyboard alone through a whole restore: 17.4 GiB written, four partitions, and **the restored Windows started unaided** |
| A write that failed part way through a restore | On one attempt the restore stopped at offset 24,265,097,216 with `Incorrect function (0x80070001)` from the guest's own storage stack. The host's log recorded no I/O error, and the same build restored the same backup successfully on the next attempt from a clean boot, so it is treated as a transient fault in the virtual machine rather than a defect. It is recorded because of how it ended: MjolnirVSS **reported the failure and refused to call the disk restored**, naming the offset, what Windows said, and that the disk would not boot. That is what a write failure is supposed to look like |
